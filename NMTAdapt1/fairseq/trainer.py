...
#                chain(self.model.encoder.parameters()),
 #                chain(self.model.decoder.parameters(), self.criterion.parameters()),
            )
        )
        params1 = list(
            filter(
                lambda p: p.requires_grad,
                chain(self.model.disc1.parameters()),
            )
        )
        params2 = list(
            filter(
                lambda p: p.requires_grad,
                chain(self.model.disc2.parameters()),
            )
        )
        if self.args.fp16 or self.args.bf16:
            ...
            if self.args.memory_efficient_fp16 or self.args.memory_efficient_bf16:
                self._optimizer = optim.MemoryEfficientFP16Optimizer.build_optimizer(
                    self.args, params,params1,params2
                )
            else:
                self._optimizer = optim.FP16Optimizer.build_optimizer(self.args, params,params1,params2)
        ...
        self._lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self.optimizer[0])
        self._lr_scheduler.step_update(0)

    def consolidate_optimizer(self):
        """For OSS, we need to consolidate the state dict."""
        if hasattr(self.optimizer[0].optimizer, "consolidate_state_dict"):
            self.optimizer[0].optimizer.consolidate_state_dict()
        if hasattr(self.optimizer[1].optimizer, "consolidate_state_dict"):
            self.optimizer[1].optimizer.consolidate_state_dict()
        if hasattr(self.optimizer[2].optimizer, "consolidate_state_dict"):
            self.optimizer[2].optimizer.consolidate_state_dict()
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            self.optimizer[0].load_state_dict(last_optim_state[0], optimizer_overrides)
            self.optimizer[1].load_state_dict(last_optim_state[1], optimizer_overrides)
            self.optimizer[2].load_state_dict(last_optim_state[2], optimizer_overrides)
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output,l3,l4,za = self.task.train_step(
                        sample=sample,
                        model=self.model,
                ...
                if self.cuda and self.get_num_updates() %100== 0:
        ...
        if self.args.fp16:
            metrics.log_scalar(
                "loss_scale",
                self.optimizer[0].scaler.loss_scale,
                priority=700,
                round=4,
                weight=0,
            )
    ...
    def clip_grad_norm(self, clip_norm):
        return self.optimizer[0].clip_grad_norm(clip_norm, aggregate_norm_fn=None)
        return self.optimizer[1].clip_grad_norm(1, aggregate_norm_fn=None)
        return self.optimizer[2].clip_grad_norm(1, aggregate_norm_fn=None)
