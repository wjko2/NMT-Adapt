...
class FairseqTask(object):
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            loss, sample_size, logging_output,l3,l4 = criterion(model, sample)
        if ignore_grad:
            loss *= 0
        #l4=l4*0
        ll3=-torch.abs(l3)
        ...
        with torch.autograd.profiler.record_function("backward"):
            ...
            if update_num%3==0:
                zz=1
            if update_num<=300.0*ff:
               va=1
            if update_num<=320.0*ff:
                za=optimizer[0].backward2(loss*1.0,retain_graph=True)
            else:
                za=optimizer[0].backward2(loss*1.0-zz*(60*ll3+60*ll4),retain_graph=True)
            lo3=l3*va
            lo4=l4*va
            
        return loss, sample_size, logging_output,lo3,lo4,za
